...
local function snap_SNITCH()
    ...
    if fileclass == "video" then
        ...
        local CALL_OS = "mpv --screen=0 --fs-screen=0 --volume=10 --start="..start_second..' "'..file_name_only..'"'
        print(CALL_OS)
        print(start_second)
        strCmd = CALL_OS
        mp.command("keypress SPACE")
...
mp.add_key_binding("HOME", "start_cut", start_cut, {repeatable=true})
mp.add_key_binding("END", "end_cut", end_cut, {repeatable=true})
